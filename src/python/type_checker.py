...
class TypeChecker:
    ...
    def check_ref(self, node):
        ...
        if node.typ is not None:
            tir_type = self.check(node.typ)
            checked_type = self.check(node.typ).checked
            assert isinstance(checked_type, types.Type), \
                f"{checked_type} not a type_defs Type"
        for value in node.values:
            checked_value = self.check(value)
            checked_type = self.update_raw_types(checked_type, checked_value.typ)
            if isinstance(value.typ, ir.SequenceType):
                value.typ = node.typ
        match node:
            case ir.FunctionRef():
                param_types = []
                for pname, ptype in zip(tir_type.param_names, checked_type.fields):
                    values = node.param_values.get(pname, [])
                    for value in values:
                        checked_value = self.check(value)
                        ptype = self.update_raw_types(ptype, checked_value.typ)
                    param_types.append(ptype)

                return_type = checked_type.fields[-1]
                for value in node.return_values:
                    checked_value = self.check(value)
                    return_type = self.update_raw_types(
                        return_type, checked_value.typ
                    )
            ...
            case ir.FieldRef():
                ...
                checked_node = None
                assert isinstance(parent.typ, types.CompoundType), \
                    f"Expected compound type, got {parent.typ}"
                if id(parent.typ) in self.fields_map \
                        and node.name in self.fields_map[id(parent.typ)]:
                    index = self.fields_map[id(parent.typ)].index(node.name)
                    checked_node = tir.FieldRef(node.name, parent, index=index)
                    raw_type = parent.typ.fields[index]
                    checked_node.typ = raw_type
                method = self.methods_map.get(id(parent.typ), {}).get(node.name)
                if method:
            ...
            case ir.IndexRef():
                ...
                checked_node = tir.IndexRef(
                    node.name, checked_parent, checked_index, typ=checked_type.checked
                )
            case ir.ConstRef():
                value = self.check(node.value)
                checked_node = tir.ConstRef(node.name, value)
                checked_type = self.update_raw_types(checked_type, value.typ)
                if checked_type.flags & types.TypeFlag.META:
                    checked_node = tir.Type(node.name, typ=checked_type, checked=value.value)
            case ir.FunctionSigRef():
                ...
                for value in node.values:
                    # TODO: this can be changed to use type_defs
                    #       if the parameter name checks are done here instead
                    checked_node = self.update_types(checked_node, self.check(value))
            case ir.StructRef():
                field_names = [fname for fname in node.fields.keys()]
                raw_type = self.get_core_type(node)
                checked_node = tir.Type(node.name, checked=raw_type)
                self.fields_map[id(raw_type)] = field_names
    ...
    def check_instr(self, node):
        match node:
            ...
            case ir.Call(ref, args):
                ...
                # TODO: this is a hack
                return_type = checked_ref.typ.fields[-1]
                checked_node.typ = return_type
            ...
            case ir.CBranch(condition, t_block, f_block):
                ...
                if checked_condition.typ != builtin.types["bool"]:
    ...
    def check_object(self, node):
        match node:
            ...
            case ir.Constant():
                ...
                # TODO: if this can be a `tir.Type`
                #       then we need to keep `.checked` by handling separately
                checked_node = tir.Constant(node.value, typ=typ.checked)
            ...
            case ir.StructLiteral():
                tir_type = self.check(node.typ)
                checked_type = tir_type.checked
                checked_fields = []
                assert list(node.fields.keys()) == self.fields_map[id(checked_type)]

                for i, fval in enumerate(node.fields.values()):
                    checked_val = self.check(fval)
                    checked_fields.append(checked_val)
                    checked_type.fields[i] = self.update_raw_types(
